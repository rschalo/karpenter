...
var _ = Describe("Validation", func() {
	...
	Context("Disruption", func() {
		...
		It("should fail to RuntimeValidate a budget with an invalid cron", func() {
		...
		It("should fail to RuntimeValidate a schedule with less than 5 entries", func() {
			nodePool.Spec.Disruption.Budgets = []Budget{{
				Nodes:    "10",
				Schedule: ptr.String("* * * * "),
				Duration: &metav1.Duration{Duration: lo.Must(time.ParseDuration("20m"))},
		...
		It("should fail to RuntimeValidate a budget with a cron but no duration", func() {
			nodePool.Spec.Disruption.Budgets = []Budget{{
				Nodes:    "10",
				Schedule: ptr.String("* * * * *"),
			}}
		...
		It("should fail to RuntimeValidate a budget with a duration but no cron", func() {
			nodePool.Spec.Disruption.Budgets = []Budget{{
				Nodes:    "10",
				Duration: &metav1.Duration{Duration: lo.Must(time.ParseDuration("20m"))},
			}}
		...
		It("should succeed to RuntimeValidate a budget with both duration and cron", func() {
			nodePool.Spec.Disruption.Budgets = []Budget{{
				Nodes:    "10",
				Schedule: ptr.String("* * * * *"),
				Duration: &metav1.Duration{Duration: lo.Must(time.ParseDuration("20m"))},
		...
		It("should succeed to RuntimeValidate a budget with neither duration nor cron", func() {
			nodePool.Spec.Disruption.Budgets = []Budget{{
				Nodes: "10",
			}}
			Expect(nodePool.RuntimeValidate()).To(Succeed())
		})
		It("should succeed to RuntimeValidate a budget with special cased crons", func() {
			nodePool.Spec.Disruption.Budgets = []Budget{{
				Nodes:    "10",
